...
function StatusBadge({ status }: { status: WishlistItem['status'] }) {
  const colorMap = { wanted: 'informative', acquired: 'success', dropped: 'danger' } as const;
  return (
    <Badge appearance="filled" color={colorMap[status] || 'informative'}>
      {t(`wishlist.status_${status}`)}
    </Badge>
  );
}

export default function WishlistItemRow({ item, onEdit, onAcquire, onDrop, onDelete }: Props) {
  const styles = useStyles();
